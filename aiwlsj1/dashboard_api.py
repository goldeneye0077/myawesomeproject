...
@router.post('/performance-targets/save', response_model=Dict[str, Any])
async def save_performance_targets(
    targets_data: Dict[str, Any],
    db: AsyncSession = Depends(get_db)
):
    """保存绩效目标"""
    try:
        print(f"[DASHBOARD API] 接收到目标数据: {targets_data}")
            ...
            if key in targets_data and targets_data[key] is not None:
                try:
                    ...
                    if value > 0:
                        ...
                        print(f"[DASHBOARD API] 添加目标: {name} = {value}")
                except (ValueError, TypeError) as e:
                    print(f"[DASHBOARD API] 跳过无效值 {key}: {targets_data[key]} - {e}")
                    continue
        
        # 提交到数据库
        await db.commit()
        print(f"[DASHBOARD API] 成功保存了 {saved_count} 个目标到数据库")
        ...
        await db.rollback()
        print(f"[DASHBOARD API] 保存失败: {str(e)}")
...
@router.get('/performance-targets/list', response_model=Dict[str, Any])
async def get_performance_targets(
    ...
    """获取绩效目标列表"""
    try:
        print(f"[DASHBOARD API] 获取绩效目标数据")
        ...
        print(f"[DASHBOARD API] 找到 {len(targets_data)} 个目标")
        ...
        print(f"[DASHBOARD API] 获取失败: {str(e)}")
